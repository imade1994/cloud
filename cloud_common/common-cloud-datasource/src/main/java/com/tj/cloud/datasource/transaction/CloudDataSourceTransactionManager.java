 ...
 **/
@Getter
public class CloudDataSourceTransactionManager extends AbstractPlatformTransactionManager implements ResourceTransactionManager, InitializingBean {

    private static Logger log = LoggerFactory.getLogger(CloudDataSourceTransactionManager.class);

    /**
     * 当前线程的顶层hlTxObject
     * 调用HlCloudDataSourceTransactionManager.addDataSource的数据源全部归于这里处理
     */
    ...
    static {
        ...
        //SEATA_PRESENT = ClassUtils.isPresent("io.seata.rm.datasource.DataSourceProxy", classLoader);
    }
    ...
    protected Object doGetTransaction() {
        ...
        if (threadLocalTopAbTxObject.get() == null) {
            ...
            if (log.isDebugEnabled()) {
                log.debug("hl 事务编号[" + hlTxObject.getSerialNumber() + "]为顶层事务开始，调用 hlCloudDataSourceTransactionManager.addDataSource 的数据源全部归于这里处理");
            }
        } else {
            if (log.isDebugEnabled()) {
     ...
     * 增加一个数据源到线程中
     */
    private static void addGlobalDataSource(String dsKey, DataSource dataSource, CloudDataSourceTransactionObject hlTxObject) {
        try {
            // 拿出dsKey的资源 txObject
            ...
            throw new CannotCreateTransactionException("在 hl 事务编号[" + hlTxObject.getSerialNumber() + "]中，" + "数据源别名[" + dsKey + "]打开连接错误", ex);
        }
    ...
    protected void doCommit(DefaultTransactionStatus status) {
        ...
        for (Map.Entry<String, DataSourceTransactionObject> entry : hlTxObject.getDsTxObjMap().entrySet()) {
            try {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("在 hl 事务编号[" + hlTxObject.getSerialNumber() + "]中，" + "数据源别名[" + entry.getKey() + "]提交事务成功");
                }
    ...
    protected void doRollback(DefaultTransactionStatus status) {
        ...
        for (Map.Entry<String, DataSourceTransactionObject> entry : hlTxObject.getDsTxObjMap().entrySet()) {
            try {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("在 hl 事务编号[" + hlTxObject.getSerialNumber() + "]中，" + "数据源别名[" + entry.getKey() + "]回滚事务成功");
                }
            } catch (SQLException ex) {
    ...
    protected void doSetRollbackOnly(DefaultTransactionStatus status) {
        ...
        for (Map.Entry<String, DataSourceTransactionObject> entry : hlTxObject.getDsTxObjMap().entrySet()) {
            if (status.isDebug()) {
                log.debug("在 hl 事务编号[" + hlTxObject.getSerialNumber() + "]中，" + "修改数据别名为 [" + entry.getKey() + "]的链接资源为 rollback-only");
            }
    ...
    protected void doCleanupAfterCompletion(Object transaction) {
        ...
        for (Map.Entry<String, DataSourceTransactionObject> entry : hlTxObject.getDsTxObjMap().entrySet()) {
            ...
            try {
                ...
                log.debug("在 hl 事务编号[" + hlTxObject.getSerialNumber() + "]中，" + "在完成事务后，数据源别名为[" + entry.getKey() + "]的属性无法被还原", ex);
            }
            if (txObject.isNewConnectionHolder()) {
                if (log.isDebugEnabled()) {
                    log.debug("在hl 事务编号[" + hlTxObject.getSerialNumber() + "]中，" + "在完成事务后，释放数据源别名为[" + entry.getKey() + "]的 jdbc 的链接");
                }
