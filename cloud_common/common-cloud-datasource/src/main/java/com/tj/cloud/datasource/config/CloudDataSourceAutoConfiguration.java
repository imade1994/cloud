...
public class CloudDataSourceAutoConfiguration {
    ...
    static {
        ...
        REPLICATE_PRESENT = ClassUtils.isPresent("com.tj.cloud.datasource.readwrite.ReadWriteSplitDataSourceBuilder", classLoader);
    }
    ...
    private static DataSource createDataSource(CloudDataSourceProperties dataSourceProperties) {
        ...
        if (ObjectUtils.isEmpty(validateQuery)) {
            ...
            Assert.isTrue(!DatabaseDriver.UNKNOWN.equals(databaseDriver), () -> dataSourceProperties.getDbType() + "  is not defined in org.springframework.boot.jdbc.DatabaseDriver");
    ...
    private DataSource createReplicateDataSource() {
        ...
        List<DataSource> replicateDataSources = FunctionUtil.streamOfNullable(dataSourceProperties.getReplicates(), false).map(CloudDataSourceAutoConfiguration::createDataSource).collect(Collectors.toList());
        return new ReadWriteSplitDataSourceBuilder().withPrimary(primaryDataSource)
                .withReplicates(replicateDataSources)
                .withLoadBalanceAlgorithmClass((Class<? extends ReplicaLoadBalanceAlgorithm>) dataSourceProperties.getReplicaLoadBalanceAlgorithmClass())
                .build(environment);
    }
    ...
    public DynamicDataSource dataSource() throws Exception {
        ...
        //默认数据源不会被过期
        targetDataSources.put(DataSourceUtil.DEFAULT_DATASOURCE, REPLICATE_PRESENT ? createReplicateDataSource() : createSeataDataSourceProxy(dataSourceProperties), ExpirationPolicy.ACCESSED, 365, TimeUnit.DAYS);
