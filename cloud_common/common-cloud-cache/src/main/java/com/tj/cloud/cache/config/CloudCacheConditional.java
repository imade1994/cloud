...
public class CloudCacheConditional extends SpringBootCondition {
    ...
    public ConditionOutcome getMatchOutcome(ConditionContext context, AnnotatedTypeMetadata metadata) {
        ...
        if (ObjectUtils.isEmpty(value)) {
            return CloudMemoryCacheConfiguration.class.getName().equals(sourceClass) ? ConditionOutcome.match() : ConditionOutcome.noMatch(value + " cache type");
        }
        value = value.replace("-", "_").toUpperCase();
        CloudCacheType cacheType = CloudCacheType.valueOf(value);
        return cacheType.getConfigurationClass().getName().equals(sourceClass) ? ConditionOutcome.match() : ConditionOutcome.noMatch(value + " cache type");
    }
}
