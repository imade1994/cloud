        ...
        for (PropertySource<?> propertySource : environment.getPropertySources()) {
            if (propertySource instanceof EnumerablePropertySource) {
                for (String propertyName : ((EnumerablePropertySource<?>) propertySource).getPropertyNames()) {
                    if (StringUtils.startsWith(propertyName, J2CacheConstant.L1_PREFIX) && filterPropertyNames.add(propertyName)) {
                        config.getL1CacheProperties().putIfAbsent(replaceJ2CachePrefix(propertyName, J2CacheConstant.L1_PREFIX), environment.getProperty(propertyName));
                    } else if (StringUtils.startsWith(propertyName, J2CacheConstant.L2_PREFIX) && filterPropertyNames.add(propertyName)) {
                        config.getL2CacheProperties().putIfAbsent(replaceJ2CachePrefix(propertyName, J2CacheConstant.L2_PREFIX), environment.getProperty(propertyName));
                    } else if (StringUtils.startsWith(propertyName, J2CacheConstant.BROADCAST_PREFIX) && filterPropertyNames.add(propertyName)) {
                        config.getBroadcastProperties().putIfAbsent(replaceJ2CachePrefix(propertyName, J2CacheConstant.BROADCAST_PREFIX), environment.getProperty(propertyName));
                    }
                }
            }
        }
