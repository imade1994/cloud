...
public class CloudPulsarMessageConfiguration {
	...
	public PulsarClient pulsarClient() throws PulsarClientException {
		return PulsarClient.builder().serviceUrl(pulsarProperties.getServiceUrl())
				...
				.tlsCertificateFilePath(pulsarProperties.getTlsFile()).ioThreads(pulsarProperties.getIoThreads())
				.listenerThreads(pulsarProperties.getListenerThreads()).build();
	}

	@Bean("pulsarProducerMap")
	public Map<String, Producer<byte[]>> pulsarClientMap(@Qualifier PulsarClient pulsarClient)
		...
		for (PulsarProperties.Producer producer : pulsarProperties.getProducerList()) {
			producerMap.put(producer.getTopicName(), pulsarClient.newProducer().topic(producer.getTopicName())
					.producerName(producer.getProducerName()).create());
		}
		return producerMap;
	}
