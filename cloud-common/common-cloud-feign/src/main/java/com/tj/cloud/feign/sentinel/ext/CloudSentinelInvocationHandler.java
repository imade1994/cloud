package com.tj.cloud.feign.sentinel.ext;
...
/**
 * * @Author codingMan_tj * @Date 2024/3/26 14:30 * @version v1.0.0 * @desc
 **/
public class CloudSentinelInvocationHandler implements InvocationHandler {

	private final static Logger log = LoggerFactory.getLogger(CloudSentinelInvocationHandler.class);
			...
			if (methodMetadata == null) {
				...
				try {
					...
					if (fallbackFactory != null) {
						...
						// 若是R类型 执行自动降级返回R
						if (CloudResult.class == method.getReturnType()) {
							log.error("feign 服务间调用异常", ex);
							return CloudResult.failed(ex.getLocalizedMessage());
						}
						else {
							throw ex;
						}
