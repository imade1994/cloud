...
/**
 * * @Author codingMan_tj * @Date 2024/3/26 11:58 * @version v1.0.0 * @desc 配置读取工具类
 **/
public class PropertyUtil {
	...
	/**
	 * 获取较松懈字符串值 PropertyUtil.getRelaxStringValue(environment,
	 * "spring.datasource.driverClassName"); PropertyUtil.getRelaxStringValue(environment,
	 * "spring.datasource.driver_class_name");
	 * PropertyUtil.getRelaxStringValue(environment,
	...
	enum Manipulation {
		...
		private static String separatedToCamelCase(String value, boolean caseInsensitive) {
			...
			for (String field : SEPARATED_TO_CAMEL_CASE_PATTERN.split(value)) {
				...
				builder.append(builder.length() != 0 ? org.springframework.util.StringUtils.capitalize(field) : field);
			}
