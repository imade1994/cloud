...
public class DynamicModelByteController {
	...
	public void saveByteByModelName(@PathVariable("modelName") String modelName)
		 ...
		 * //this.getClass().getClassLoader().getResource(); File entityFile = new
		 * File("E:\\classes\\com\\hlxd\\dynamic\\entity\\Test.class");
		 *//*
			 * URL url = entityFile.toURI().toURL(); URL[] urls = new URL[]{url};
			 * ClassLoader classLoader = new URLClassLoader(urls); Class<?> cls =
			 * classLoader.loadClass("com.hlxd.dynamic.entity.Test");
			 * log.info("load class {} from mysql ",cls.getName());
			 *//*
				 * byte[] entityBytes = getByteStream(outputStream,entityFile); File
				 * mapperFile = new
				 * File("E:\\classes\\com\\hlxd\\dynamic\\mapper\\TestMapper.class");
				 * byte[] mapperBytes = getByteStream(outputStream,mapperFile); // 读取
				 * service文件 File serviceFile = new
				 * File("E:\\classes\\com\\hlxd\\dynamic\\service\\ITestService.class");
				 * byte[] serviceBytes = getByteStream(outputStream,serviceFile); // 读取
				 * serviceImpl文件 File serviceImplFile = new File(
				 * "E:\\classes\\com\\hlxd\\dynamic\\service\\impl\\TestServiceImpl.class"
				 * ); byte[] serviceImplBytes =
				 * getByteStream(outputStream,serviceImplFile); // 读取 controller文件 File
				 * controllerFile = new File(
				 * "E:\\classes\\com\\hlxd\\dynamic\\controller\\TestController.class");
				 * byte[] controllerBytes = getByteStream(outputStream,controllerFile);
				 * try{ outputStream.close(); }catch (Exception e){
				 * log.error("关闭文件流异常！"+e.getMessage()); } String id = new
				 * Snowflake().nextIdStr();
				 * log.info("创建模板 {} ，新建ID 为{} 的 记录",modelName,id); DynamicModel
				 * model=DynamicModel.builder() .modelName("t_test").id(id)
				 * .entityByte(entityBytes).mapperByte(mapperBytes)
				 * .serviceByte(serviceBytes).serviceImplByte(serviceImplBytes).
				 * controllerByte(controllerBytes) .entityName("Test")
				 * .serviceName("TestService") .controllerName("TestController")
				 * .mapperName("TestMapper") .serviceImplName("TestServiceImpl") .build();
				 * dynamicModelService.save(model);
				 */
	}

	private byte[] getByteStream(ByteArrayOutputStream outputStream, File mapperFile)
			throws IOException, ClassNotFoundException {
		InputStream in;
		int length;
	...
	public void codeGenerate(String tableName) {
		FastAutoGenerator.create("jdbc:mysql://10.10.103.83:2883/test?serverTimezone=GMT%2b8", "root@qrcode#qrcode",
				"Hlxd@123456").globalConfig(builder -> {
					builder.author("CodingManTJ").enableSwagger().fileOverride().outputDir("E:\\");
				}).packageConfig(builder -> {
					builder.parent(BASE_PARENT_PATH).moduleName(BASE_MODEL);
					// .pathInfo(Collections.singletonMap(OutputFile.mapperXml,BASE_RESOURCE_PATH));
				}).strategyConfig(builder -> {
					builder.addInclude(tableName).addTablePrefix("t_").entityBuilder().serviceBuilder().mapperBuilder()
							.enableBaseResultMap().controllerBuilder().enableRestStyle();
